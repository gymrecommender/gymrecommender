...
create table if not exists user_token (
	...
	updated_at TIMESTAMPTZ not null check (updated_at <= NOW() and updated_at > created_at) default NOW(),
	expires_at TIMESTAMPTZ check (expires_at > created_at) not null ,
	primary key(id),
	unique(user_id),
	unique(outer_token),
	foreign key (user_id) references account(id) on delete cascade
...
(
  ...
  primary key (id),
  unique (user_id, gym_id),
  FOREIGN KEY (user_id) REFERENCES gym(id) on delete cascade,
  FOREIGN KEY (gym_id) REFERENCES account(id) on delete cascade
);
